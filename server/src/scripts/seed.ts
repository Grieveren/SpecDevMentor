...
async function main() {
  ...
  const _designModule = await prisma.learningModule.upsert({
  ...
  const _sampleProject = await prisma.specificationProject.create({
    data: {
...
- [ ] 3. Build product catalog system
  ...
  const _blogProject = await prisma.specificationProject.create({
    data: {
