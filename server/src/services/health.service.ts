  ...
  private async checkDiskSpace(): Promise<HealthCheck> {
    try {
      ...
      return {
        ...
        status: 'warn',
        time: new Date().toISOString(),
        output: `Could not check disk space: ${error.message}`,
        details: {
          error: error.message,
        },
      };
  ...
  private async checkExternalServices(): Promise<HealthCheck> {
    ...
    if (process.env.OPENAI_API_KEY) {
      try {
        ...
        checks.push({ service: 'openai', status: 'fail', error: error.message });
        overallStatus = 'fail';
      }
    }
    ...
    if (process.env.SMTP_HOST) {
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          socket.connect(parseInt(process.env.SMTP_PORT || '587'), process.env.SMTP_HOST, () => {
        ...
        checks.push({ service: 'smtp', status: 'warn', error: error.message });
